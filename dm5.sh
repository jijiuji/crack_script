...
read llwsapp
if [ $llwsapp == "1" ];then
echo -e "[请输入您的授权域名 输入IP就行了  不要加端口和http://]"
	read domain
	echo -e "[请输入您的web流控端口]"
	read port
	echo -e "[请输入您的APP授权码（32位长度,自定义）]"
	read app_key
	curl "http://www.78wo.cn/api/check.php?domain=$domain&key=$app_key&t=dingd.cn" >> tmp.txt
	read status < tmp.txt
	rm tmp.txt
...
cd  /home/wwwroot/default/
wget http://78wo.cn/${Vpnfile}/app_api.zip && unzip -o app_api.zip >/dev/null 2>&1 #全新安装 覆盖全部文件
rm app_api.zip
