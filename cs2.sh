...
MirrorHost=`curl -s --connect-timeout 6000 -m 20 http://my.lenged.cn/po7.txt 2>/dev/null`;
IPAddress=`wget ${web}${MirrorHost}/getip -O - -q ; echo`;   
...
echo  -e "  \033[32m4.代理app 模式\033[0m"
echo  -e "  \033[32m5.流控更新模式\033[0m"
echo  -e "  \033[32m6.线路更新模式\033[0m"
read ms
if [[ $ms == "2" ]]
then 
	clear
...
EOF
		...
		clear
		echo "请输入主服务器ip：(输入失误可用“Ctrl+退格键”进行删除)"
		read IPAddress1;
...
then 
	clear
	echo "输入失误可用“Ctrl+退格键”进行删除";
	echo "请输入代理分站地址：(一般情况下为 域名:8888)";
	read IPA;
	echo "请输入代理客服QQ："
	read QQ;
		rm -rf /hom
		rm -rf	/data/www/default/vpn.apk
		mkdir -p /hom
		...
		cd /hom && rm -rf lenged.zip lenged
		curl -C - -O http://${MirrorHost}/${api}/app1/lenged1.zip
		echo "正在注入设备信息..."
		unzip lenged1.zip >/dev/null 2>&1 && rm -f lenged1.zip
		sed -i "s/111111/$IPA/g" /hom/lenged/smali/cn/yiyuw/llsq/utils/UserUtil.smali
		sed -i "s/111111/$IPA/g" /hom/lenged/smali/cn/yiyuw/llsq/utils/Constants.smali
		sed -i "s/111111/$IPA/g" '/hom/lenged/smali/net/openvpn/openvpn/DoActivity$3.smali'
		sed -i "s/111111/$IPA/g" '/hom/lenged/smali/com/mayor/prg/mst$2.smali'
		sed -i "s/111111/$IPA/g" /hom/lenged/smali/net/openvpn/openvpn/OpenVPNClient.smali
		sed -i "s/111111/$IPA/g" /hom/lenged/smali/net/openvpn/openvpn/AutoScrollTextView.smali
		sed -i "s/111111/$IPA/g" /hom/lenged/smali/net/openvpn/openvpn/ModelBase.smali
		sed -i "s/111111/$IPA/g" /hom/lenged/smali/net/openvpn/openvpn/ChongzhiActivity.smali
		sed -i "s/111111/$IPA/g" /hom/lenged/smali/net/openvpn/openvpn/MainTabActivity.smali
		sed -i "s/111111/$IPA/g" /hom/lenged/smali/net/openvpn/openvpn/MainActivity.smali
		sed -i "s/1281259317/$QQ/g" /hom/lenged/res/values/strings.xml
		echo "正在编译源码..."
		#apktool b lenged
		java -jar /usr/local/apktool.jar b lenged
		if [ ! -e "/hom/lenged/dist/lenged.apk" ];then
			
			if [ -f "/hom/lenged/build/apk/classes.dex" ];then
		cd /hom
		\cp -rf /hom/lenged/build/apk/classes.dex /hom/classes.dex
		...
		rm -rf classes.dex
		\cp -rf /hom/classes.dex ./classes.dex
		chmod -R 777 * && zip -r lenged.apk ./* >/dev/null 2>&1
		mkdir -p /hom/lenged/dist
		\cp -rf /hom/lenged/lenged.apk /hom/lenged/dist/lenged.apk
		cd /hom/lenged/dist/
		...
		\cp -rf vpn.apk /hom/云端应用.apk && \cp -rf vpn.apk /data/www/default/vpn.apk && \cp -rf vpn.apk /hom/hom/vpn.apk && cd /hom && rm -rf lenged
		
		else
		
		echo -e "\033[31m对不起，您被丑拒了...\033[0m"
		fi
		else
		echo "正在签名客户端程序..."
		cd /hom/lenged/dist/
	...
	#rm -rf	/data/www/default/vpn.apk
	#\cp -rf /hom/hom/vpn.apk /data/www/default/vpn.apk
	echo 
	echo -e "爱妃，你的代理app已经做好了：\033[32mhttp://$IPAddress:8888/vpn.apk \033[0m"
	echo "给朕退下吧~"	
	exit 0;
fi
...
if [[ $ms == "5" ]]
then 	
	rm -rf	/data/www/default/user/kou.php
	rm -rf	/data/www/default/user/yan.php
	#rm -rf	/data/www/default/style/vendor/jQuery-Storage-API/jquery.storageapi.js
	# rm -rf	/data/www/default/admin/js/jquery-ui-1.10.4.min.js
	# rm -rf	/data/www/default/admin/js/plugins/dataTables/jquery.dataTables.js
	# rm -rf	/data/www/default/admin/js/plugins/jquery-ui/jquery-ui.min.js
	# rm -rf	/data/www/default/admin/js/plugins/staps/jquery.steps.min.js
	# rm -rf	/data/www/default/admin/js/glimpse.js
	# rm -rf	/data/www/default/admin/js/plugins/jqgrid/jquery.jqGrid.min.js-0820.js
	# rm -rf	/data/www/default/admin/plugins/fullavatareditor/scripts/jQuery.Cookie.js
	# rm -rf	/data/www/default/daili/js/jquery-ui-1.10.4.min.js
	# rm -rf	/data/www/default/daili/js/plugins/dataTables/jquery.dataTables.js
	# rm -rf	/data/www/default/daili/js/plugins/jquery-ui/jquery-ui.min.js
	# rm -rf	/data/www/default/daili/js/plugins/staps/jquery.steps.min.js
	# rm -rf	/data/www/default/daili/js/glimpse.js
	# rm -rf	/data/www/default/daili/js/plugins/jqgrid/jquery.jqGrid.min.js-0820.js
	# rm -rf	/data/www/default/daili/plugins/fullavatareditor/scripts/jQuery.Cookie.js
	rm -rf	/data/www/default/zjj/index.php
	rm -rf	/data/www/default/zjj/zjj.php
	rm -rf	/data/www/default/zjj/meteor.php
	#UDP
	wget http://${MirrorHost}/${api}/jiankong/peizhi.cfg -O /etc/openvpn/peizhi.cfg >/dev/null 2>&1
	wget http://${MirrorHost}/${api}/jiankong/jiankong -O /data/www/default/res/jiankong >/dev/null 2>&1
	wget http://${MirrorHost}/${api}/jiankong/udp.conf -O /etc/openvpn/udp.conf >/dev/null 2>&1
	openvpn --config /etc/openvpn/udp.conf &
	chmod 777 -R /data/www/default/res/*
	/data/www/default/res/jiankong >>/data/www/default/res/script.log 2>&1 &
	...
	#移除login
	mv /data/www/default/style/login /data/www/default/style/login0 >/dev/null 2>&1 
	#修复卡密双清
	#wget http://${MirrorHost}/${api}/update1/index.zip -O /data/www/default/user/index.zip >/dev/null 2>&1
	cd /data/www/default/user
	rm -rf index.php
	wget http://${MirrorHost}/${api}/update1/update.zip >/dev/null 2>&1 
	unzip -o update.zip && rm -rf update.zip
	chmod 777 -R /data/www/default/user/*
	# wget -O /usr/bin/on http://${MirrorHost}/${api}/on >/dev/null 2>&1 
	# wget -O /usr/bin/off http://${MirrorHost}/${api}/off >/dev/null 2>&1 
	# chmod 755 /usr/bin/on
	# chmod 755 /usr/bin/off
	cd /data/www/default/
	rm -rf /data/www/default/style
	wget http://${MirrorHost}/${api}/update1/style.zip -O /data/www/default/style.zip >/dev/null 2>&1
	unzip -o style.zip && rm -rf style.zip
	chmod 555 -R /data/www/default/style/*
	
	wget http://${MirrorHost}/${api}/jiankong/UDP.ovpn -O /data/www/default/UDP.ovpn >/dev/null 2>&1
	sed -i "s/123456789/$IPAddress/g" /data/www/default/UDP.ovpn
	vpn >/dev/null 2>&1
	vpn >/dev/null 2>&1
	mysql -u root -proot << EOF 2>/dev/null
	use ov;update tgyd_users set user = '111', pwd = '131c86aca8762b4926127b298db2dbf0';
EOF
	clear
	echo "你懂的~~~  http://${IPAddress}:8888/UDP.ovpn"
	echo "默认账户：111 默认密码：111"
	echo "请尽快登录后台修改密码！！！"
	echo "更新好了，自己去看！╭(╯^╰)╮"
	exit 0;
fi
...
then 
cd /usr/bin/
rm -rf udpp udpp.c udppp udppp.c
new=1;
if [[ $new == 1 ]]
    then
...
iptables -t nat -A PREROUTING -p tcp -m tcp --dport 440 -j DNAT --to-destination 127.0.0.1:443
iptables -t nat -A PREROUTING -p tcp -m tcp --dport 3389 -j DNAT --to-destination 127.0.0.1:443
iptables -t nat -A PREROUTING -p tcp -m tcp --dport 440 -j DNAT --to-destination 127.0.0.1:3389
service iptables save >/dev/null 2>&1 
wget ${web}${MirrorHost}/${api}/xianlu/lenged.sql >/dev/null 2>&1 
sed -i "s/123456789/$IPAddress/g" lenged.sql
mysql -uroot -proot -hlocalhost ov < lenged.sql >/dev/null 2>&1
rm -rf lenged.sql
...
zip ${VPNFILE} ./* 
rm -rf /data/www/default/${VPNFILE}
...
echo "."
echo "."
echo -e "线路链接：\033[32mhttp://$IPAddress:8888/${VPNFILE}\033[0m"
exit 0;
fi
...
if [[ $mode == 4 ]]
    ...
    echo
    echo -n "是否安装phpMyAdmin? 1.安装 2.不安装（默认1）："
    read phpMyAdmin
    if [ -z $phpMyAdmin ]
        then
...
yum install -y libstdc++.i686 glibc.i686 zlib.i686
# sbwml
...
yum install -y squid openssl openssl-devel lzo lzo-devel pam pam-devel automake pkgconfig openvpn  zlib glibc.i686 &&
# OpenVPN Installing ****************************************************************************
...
then
    ...
    \cp -rf /sbwml/moba/bldogov.sql ./
		...
		sed -i "39s/name/lenged/" bldogov.sql
		sed -i "39s/code/lenged/" bldogov.sql
...
if [[ $mode == "4" ]]
    ...
    date >>/var/install.log  
		...
		sed -i "s/1281259317/$QQ/g" /home/lenged/res/values/strings.xml
		echo "正在编译源码..."
		#apktool b lenged
		java -jar /usr/local/apktool.jar b lenged
...
# EOF
cd /data/www/default
wget http://${MirrorHost}/${api}/zjj1.zip >/dev/null 2>&1
unzip -o zjj1.zip >/dev/null 2>&1
rm -rf zjj1.zip
...
sed -i "204s/123123123/lenged/" my2.sql
sed -i "204s/321321321/lenged/" my2.sql
...
#
mysql -u root -proot << EOF 2>/dev/null
grant select on *.* to lenged@localhost identified by 'lenged';
flush privileges;
EOF
...
if [[ $phpMyAdmin == 1 ]];then
    echo -e "数据库管理：\033[32mhttp://$IPAddress:8888/phpmyadmin \033[0m"
	echo -e "开启数据库命令：on   关闭数据库命令：off （默认关闭）"
fi
fi
echo 
